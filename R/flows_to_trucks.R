...
truck_allocation_factors <- read.csv("../data/trucks/truckallocation.csv") %>%
...
# instead.
truck_equivalency_factors <- read.csv("./data/trucks/truckequivalence.csv") %>%
...
empty_truck_factors <- read.csv("./data/trucks/emptytrucks.csv") %>%
...
load("./data/OriginalFAFData.Rdata")
...
save(FAF, file = "./data/processedfafdata.Rdata")
