...
disaggregating these region-to-region flows into county-to-county flows. All 
methods have strengths and weaknesses; this new method is designed with a few 
specific goals in mind:
  ...
  (R   and Python), and is available on the open internet and hosted on 
  [GitHub](https://github.com/gregmacfarlane/trucksim_disagg). Datasets used are
  clearly referenced or linked to, and are free to the general public. 
  Constructive comment and contributions are sought and welcome from 
  all.^[Ideally, with a pull request on GitHub.]
...
# Discretization 
Version 3 of the FAF documentation included a 
...
There are three types of factors included with this methodology:
 ...
 1. Table 3.3 contains allocation factors that determine the truck configuration
 (single-unit, combination-semi, etc.) based on the distance traveled.
 2. Table 3.5 contains empty truck factors that determine the percent of empty
 trucks traveling between two FAF regions based on the configuration and the
 whether the shipment is domestic or a land-border shipment.^[NAFTA allows
 trucks to carry goods across borders, but not necessarily return full; hence, 
 international shipments have a greater percentage of empty trucks.]
 3. Appendix A contains payload equivalency factors that determine how much of
 each commodity a truck of a given configuration and vehicle type can carry.
...
calendar year. The simulation framework used in the disaggregation step requires
...
industry are represented in Figure \label{fig:maketable}.
...
graphically in Figure \ref{usetable}.
...
are the zone's socioeconomc variables. 
...
do so. The size term for airports is based on the 2006-2010
